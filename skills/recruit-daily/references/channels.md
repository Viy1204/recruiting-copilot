...
- **猎聘封境外 IP**：开着 VPN / 整机隧道（Outline、Clash TUN 等）时，就算能扫码登录，
  接口也会拦「检测到您当前网络地址在境外」。用猎聘前关 VPN，或把 `*.liepin.com` 分流直连。
- **猎聘页被清成 about:blank**：liepin-cli < 0.2.4 的已知问题（安全脚本在页面加载瞬间检测
  CDP 会话的 Runtime 域），`npm update -g @viyzhu/liepin-cli` 到 ≥ 0.2.4 即根治。
