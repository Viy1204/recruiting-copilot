...
- **"能发出去"和"能给人看"是两回事**。日报除了存成文档，还能推一张飞书卡片到手机上。卡片这东西有个陷阱：格式检查工具（校验器）说"合法"，不等于它长得能看——只有标题加几大段文字的卡，机器完全放行，人看了却抓不到重点。所以卡片这步写了两道关：先过机器校验，再过四个自问（三秒能读出结论吗？数字有没有单独成格？状态标签是不是真状态？单段文字是不是一屏能扫完？）。第二道关只能靠自查，写进文档才不会被跳过。
