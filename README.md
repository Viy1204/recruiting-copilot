...
> ### ⚠️ 账号安全须知（2026-08-18 更新，用 DSH 面板的请务必读）
>
> **日常招聘动作请一律走 CLI 命令，别在「招聘浏览器」面板里手动点操作。**
>
...
> 本版本已做的加固：
>
> - **摘掉有头/无头切换按钮**——需要真窗口请用 `RECRUIT_BROWSER_HIDDEN=false` 重启浏览器，
>   不要在同一会话内切模式。
...
两只浏览器**默认无头**：不弹到前景、不抢键盘焦点，它在后台干活时你可以照常用别的软件。
想看它在做什么，看这个面板就是了——这正是面板存在的理由。面板默认开着「贴合」，
把页面视口固定成按源写死的尺寸（BOSS `958×1149`、猎聘 `1440×1149`），
所以面板里的文字是原生大小，而不是整页缩小后的糊图。

确实需要一个真窗口时：设 `RECRUIT_BROWSER_HIDDEN=false`（本插件 / boss-cli / liepin-cli 三方共读）。
端口上已经有实例在跑时改这个变量不生效——那只会被直接复用，得先 `boss shutdown` / `liepin quit` 关掉它。
