...
给 HR / 猎头用的一套 AI 招聘工作流：**逼问式梳理岗位真实要求 → 每日 Boss直聘 + 猎聘双通道寻源初筛 → 打招呼 → 候选人台账 → 日报**。
...
- **安全红线**：对外不可逆动作（打招呼、点"不合适"）默认先经你确认，绝不自动拒人。
...
3. 各扫码登录一次（登录态持久化）：
   ...
   liepin login
   ```
4. **可选**：飞书用户装 lark-cli 并配置凭证 → 日报直接出飞书云文档；不装则日报输出本地 Markdown，功能不受影响。
...
> 补充：支持 `~/.agents/skills` 约定的工具，也可以用 `npx skills add <本仓库>` 把三个 skill
> 装成全局（仓库需可访问）。不装也没关系——工作区里已经带了一份。
