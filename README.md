...
给 HR / 猎头用的一套 AI 招聘工作流：**逼问式梳理岗位真实要求 → 每日 Boss直聘 + 猎聘双通道寻源初筛 → 打招呼 → 约面试 → 候选人台账 → 日报**，外加市场人才盘点、本地简历评估和飞书邮箱简历按需收取。
...
- **简历收取与评估**：可直接丢本地简历，也可检查飞书邮箱近期来自猎聘/BOSS 的简历邮件，去重下载附件后接入同一套 review、台账和面试档案流程。
...
3. 各扫码登录一次（登录态持久化）：
   ...
   liepin login
   ```
4. **可选**：飞书用户装 lark-cli 并配置凭证 → 日报出飞书云文档、约面试直接建日历日程和视频会议，也能从飞书邮箱收取猎聘/BOSS 简历附件；
   不装则日报输出本地 Markdown、约面试给你一份手动建会清单、简历需本地提供。寻源、本地简历 review 和台账功能不受影响。
...
> 并把本仓库 `skills/` 注册为全局自定义 skill 根；同包还带 host 插件（CDP 抓帧）
> 与客户端模块（右侧面板）。
